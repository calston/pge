...
{not used for drawing, only for data...}
  ...
  ClearBlit            = 8;            {does not copy color number 0}
{draw modes}
  PixelDrawing          = 0;
  LineDrawing           = 1;
  FillDrawing           = 2;
...
PROCEDURE PutText(X,Y:INTEGER;S:string;Proportional:BOOLEAN;
...
Var MouseHide:boolean;

IMPLEMENTATION


USES
  Crt, {!!may cause divide by zero error on Pentium computers!!}
  DOS,strings;
...
{used for switching correctly between text and graphical mode}
  VideoModeStored    :BOOLEAN    = FALSE;
...
{used for switching correctly between text and graphical mode}
  SVGAModeBufferSize :WORD;
  SVGAModeBufferPtr  :POINTER;
...
{special clipping routine for a linef; advanced coding!
...
{use it}
  ...
  BEGIN
    ...
    END
    ELSE StrPCopy(LocalStr,S);             {also copy to local}
...
{       If BitBlit<8 then}
      ...
      {Err it may be optimized, but what about transparency?
       Colins sneeky hack follows}
